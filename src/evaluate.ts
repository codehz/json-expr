import { z } from "zod"
import type { CompiledData, Variable } from "./types"
 ...
 * ```
 */
export function evaluate<TResult>(
  data: CompiledData,
  ...
  if (!evaluator) {
    // 构造求值函数
    const functionBody = buildEvaluatorFunctionBody(
 ...
 * ```
 */
function buildEvaluatorFunctionBody(
  expressions: string[],
