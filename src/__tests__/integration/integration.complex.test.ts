...
describe("集成测试：复杂表达式组合", () => {
  describe("对象属性与函数组合", () => {
    test("函数调用对象属性", () => {
      ...
      expect(
        compileAndEvaluate<number>(combinedExpr, { double, obj }, { double: (n: number) => n * 2, obj: { value: 15 } })
      ).toBe(30);
    });
    ...
    test("嵌套对象属性运算", () => {
      ...
      expect(compileAndEvaluate<number>(ageCalcExpr, { obj }, { obj: { user: { name: "Alice", age: 30 } } })).toBe(70);
    });
  });
  ...
  describe("条件与计算混合", () => {
    test("条件表达式中使用复杂计算", () => {
      ...
      expect(compileAndEvaluate<number>(e, { x, y }, { x: 5, y: 3 })).toBe(22); // 25 - 3
      expect(compileAndEvaluate<number>(e, { x, y }, { x: 3, y: 5 })).toBe(22); // 25 - 3
    });
  ...
  describe("表达式依赖链", () => {
    test("多层依赖的复杂表达式", () => {
      ...
      // x=5, y=3: 2*25 - 9 = 41
      expect(compileAndEvaluate<number>(final, { x, y }, { x: 5, y: 3 })).toBe(41);
      // x=10, y=4: 2*100 - 16 = 184
      expect(compileAndEvaluate<number>(final, { x, y }, { x: 10, y: 4 })).toBe(184);
    });
    ...
    test("菱形依赖结构", () => {
      ...
      expect(compileAndEvaluate<number>(result, { x }, { x: 5 })).toBe(99);
    });
  });
  ...
  describe("实际场景模拟", () => {
    test("价格计算", () => {
      ...
      expect(
        compileAndEvaluate<number>(
          ...
          { price: 100, quantity: 3, discount: 0.1, taxRate: 0.08 }
        )
      ).toBeCloseTo(291.6);
    });
    ...
    test("坐标变换", () => {
      ...
      expect(compileAndEvaluate<number>(distance, { x, y, angle }, { x: 3, y: 4, angle: Math.PI / 4 })).toBeCloseTo(
        original
      );
      expect(compileAndEvaluate<number>(distance, { x, y, angle }, { x: 3, y: 4, angle: Math.PI })).toBeCloseTo(
        original
      );
    });
    ...
    test("条件评分系统", () => {
      ...
      expect(compileAndEvaluate<string>(grade, { score, bonus, penalty }, { score: 85, bonus: 10, penalty: 5 })).toBe(
        "A"
      ); // 90
      expect(compileAndEvaluate<string>(grade, { score, bonus, penalty }, { score: 75, bonus: 0, penalty: 0 })).toBe(
        "C"
      );
      expect(compileAndEvaluate<string>(grade, { score, bonus, penalty }, { score: 50, bonus: 0, penalty: 0 })).toBe(
        "F"
      );
      expect(compileAndEvaluate<string>(grade, { score, bonus, penalty }, { score: 95, bonus: 20, penalty: 0 })).toBe(
        "A"
      ); // clamped to 100
    });
  });
