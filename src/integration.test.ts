...
test("集成测试：嵌套对象属性访问", () => {
  // 测试创建嵌套对象并测试多层属性链访问
  const profile = variable(z.object({
    user: z.object({
      name: z.string(),
      age: z.number(),
    }),
  ...
  expect(evaluate<string>(userNameCompiled, { 
    profile: { 
  ...
  expect(evaluate<number>(userAgeCompiled, { 
    profile: { 
      user: { name: "Alice", age: 30 }, 
      settings: { theme: "dark" } 
    } 
  ...
  expect(evaluate<string>(themeCompiled, { 
    profile: { 
      user: { name: "Alice", age: 30 }, 
      settings: { theme: "dark" } 
    } 
...
test("集成测试：对象内数组方法", () => {
  ...
  const testObj = {
    numbers: [1, 2, 3, 4, 5],
    sum: function() { return this.numbers.reduce((a: number, b: number) => a + b, 0) },
  }
...
test("集成测试：多个对象方法调用", () => {
  ...
  const testData = {
    ...
    getMax: function() { return Math.max(...this.numbers) },
    getMin: function() { return Math.min(...this.numbers) },
    getAverage: function() { return this.numbers.reduce((a: number, b: number) => a + b, 0) / this.numbers.length },
  }
